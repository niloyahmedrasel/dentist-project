...
if (isset($_POST['action'])) {
  ...
  if ($action == 'submit') {
    $sql = "INSERT INTO appointment_request (name, gender, phone, dob, address, email, previous_attendance, appointment_type,doctor_select,status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param('ssssssssss', $name, $gender, $phone, $dob, $address, $email, $previous_attendance, $appointment_type,$doctor_id, $status);
    $result = $stmt->execute();
